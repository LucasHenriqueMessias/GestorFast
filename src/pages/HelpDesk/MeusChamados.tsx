...
const MeusChamados = () => {
  ...
  const fetchChamados = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/tab-helpdesk/solicitante/${username}`, {
        headers: {
                ...
                <Table>
                  <TableHead>
                    <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                      ...
                      <TableCell sx={{ fontWeight: 'bold', cursor: 'pointer' }} onClick={() => handleSortChange('Responsavel')}>
                        Responsável {sortConfig.field === 'Responsavel' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                      </TableCell>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
                  <Box>
                    <Typography variant="body2" color="text.secondary">
                      ...
                      Responsável: {selectedChamado.Responsavel}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="body2" color="text.secondary">
                  ...
                  <Box>
                    ...
                    {editMode ? (
                      <TextField
                        fullWidth
                        type="date"
                        value={editFormData.Expectativa_Conclusao}
                        onChange={(e) => handleEditFormChange('Expectativa_Conclusao', e.target.value)}
                        size="small"
                        sx={{ mt: 1 }}
                      />
                    ) : (
